...
家族が増えることを見据え、家計管理をより本格的に行いたいという個人的な動機から本プロジェクトを作成しました。

収支を記録・集計するだけでなく、AIが実際のデータをもとにアドバイスを返してくれる仕組みを作ることで、「記録して終わり」ではなく「改善につながる家計管理」を実現することを目指しました。

> 📌 現時点はバックエンドAPIとして完結しています。フロントエンド（React等）との連携を想定してCORS設定済みで、フロントエンドの実装は今後の予定です。
